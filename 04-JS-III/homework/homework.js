...
function numeroMasGrande(numeros) {
  ...
  {
    if (mayor < numeros[i+1]){mayor = numeros[i+1];}
  }
  return mayor;
}
...
function empiezaConNueve(n) {
  ...
  var aux = n / Math.pow(10,(numero.length-1));
  if (Math.floor(aux) === 9) {return true;}
  else return false;
}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  0
  var cantidad = 1;
  for (let i=0; i<arreglo.length-1; i++)
  {
    if (arreglo[i] === arreglo[i+1]){cantidad++}
  }
  if (cantidad === arreglo.length) {return true;}
  else return false;
} 
...
if (cantidad === 3){return nuevoArray;}
